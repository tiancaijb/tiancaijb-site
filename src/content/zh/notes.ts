...
这是我写的一个 Doom Emacs 模块，读取 org-clock 的记录，自动生成一份视觉化的时间轴 HTML。

## 数据来源：org-clock

在 Emacs 里做任何事时，我会用 org-mode 的 clock 功能记录时间：
...
按 \`SPC c i\`（org-clock-in）开始计时，\`SPC c o\`（org-clock-out）停止。Doom Emacs 默认绑定，Emacs 自动记录时间跨度。

## 一键生成 HTML 时间轴

运行 \`M-x tlh-show\`（或绑定快捷键 \`SPC t h\`），脚本自动：

1. 扫描 \`~/org/\` 下所有 .org 文件
2. 提取今天所有的 CLOCK 记录
3. 解析任务层级（含清理：去 TODO/DONE、优先级、标签）
4. 合并相邻同任务的时间段
5. 按任务名子串匹配颜色（代码=红、学习=绿、色情=粉 等）
6. 生成深色主题 HTML，6AM-11PM 可视化时间轴
7. 用 Windows 浏览器打开

### 时间轴长这样

深色背景，左侧时间刻度（6AM-11PM），每个 CLOCK 区块按时间比例定位，鼠标悬停高亮。颜色按任务类型区分，一眼就能看出今天的时间花在哪了。
...
- **不打断心流**——开始/停止 clock 只需两个快捷键，不离开 Emacs
- **数据自动**——时间记录是 org-clock 的副产品，不需要手动填
- **可视化**——HTML 比 org 文本直观得多，一眼看清时间花在哪
- **和 org-agenda 联动**——TODO 和 CLOCK 在同一套系统里
